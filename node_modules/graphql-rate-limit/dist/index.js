...
var getGraphQLRateLimiter = (userConfig) => {
  ...
  const rateLimiter = async ({
    ...
    return limitReached ? errorMessage : void 0;
  };
  return rateLimiter;
};
  ...
  class GraphQLRateLimit extends _utils.SchemaDirectiveVisitor {
    ...
    visitFieldDefinition(field) {
      ...
      field.resolve = async (parent, args, context, info) => {
        ...
        if (errorMessage) {
          throw customConfig.createError ? customConfig.createError(errorMessage) : new RateLimitError(errorMessage);
        }
...
var createRateLimitRule = (config) => {
  ...
  return (fieldConfig) => _graphqlshield.rule.call(void 0, {cache: "no_cache"})(async (parent, args, context, info) => {
    const errorMessage = await rateLimiter({
      parent,
      args,
      context,
    ...
    if (errorMessage) {
      return config.createError ? config.createError(errorMessage) : new RateLimitError(errorMessage);
    }
    return true;
  });
};
